...
function renderCustomerSpeechIndicator(progressVisible) {
  ...
  const checkpoints = [
    ...
    { buttonKey: "appointment", label: "入庫日時案内", keys: ["proposed_appointment"], done: passed("proposed_appointment"), na: notApplicable("proposed_appointment"), staffText: "8月30日午前10時はいかがでしょうか。", responseText: "では、その日でお願いします。", audioId: "inspection_proposed_appointment_customer" },
    { buttonKey: "waiting", label: "来店・待ち方確認", keys: ["explained_duration_and_wait", "confirmed_waiting"], done: passed("explained_duration_and_wait") || passed("confirmed_waiting") || anyStaffText(explainsWaiting), na: notApplicable("explained_duration_and_wait") && notApplicable("confirmed_waiting"), staffText: "当日はご来店いただき、店内でお待ちになりますか。", responseText: "出かける可能性があるので、一応代車を用意してほしいんですが、できますか？", audioId: "inspection_waiting_followup_loaner_request" },
    { buttonKey: "loaner", label: "代車案内", keys: ["explained_loaner"], done: passed("explained_loaner"), na: notApplicable("explained_loaner"), staffText: "早めのご予約ですので、代車をご用意できます。", responseText: "お願いします。", audioId: "inspection_booking_invitation_accept_customer" },
    ...
    { buttonKey: "recap", label: "入庫日時最終確認", keys: ["recapped_appointment"], done: passed("recapped_appointment"), na: notApplicable("recapped_appointment"), staffText: "佐藤様、8月30日午前10時、代車をご用意してお待ちしております。", responseText: "お願いします。", audioId: "inspection_recapped_appointment_customer" },
