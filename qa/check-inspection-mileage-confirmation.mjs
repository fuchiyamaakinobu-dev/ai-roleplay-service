...
assert.match(
  appSource,
  /if \(asksCurrentMileage\(decisionText\) \|\| asksCurrentMileage\(text\)\)[\s\S]*?askedDurationAlready = state\.inspectionDurationQuestionAsked[\s\S]*?inspection_current_mileage_customer[\s\S]*?inspection_current_mileage_and_duration_customer/,
...
const mileageReplyIndex = appSource.indexOf("if (asksCurrentMileage(decisionText) || asksCurrentMileage(text))");
