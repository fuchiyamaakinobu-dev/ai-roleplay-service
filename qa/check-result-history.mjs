...
assert.match(historyPageSource, /id="deleteEmployeeButton"[^>]+disabled/);
assert.match(historySource, /where\("employeeCode", "==", employeeCode\)/);
assert.match(historySource, /deleteDocumentsInBatches\("roleplayResults", resultSnapshot\.docs\)/);
assert.match(historySource, /deleteDocumentsInBatches\("roleplayActivity", activitySnapshot\.docs\)/);
