...
assert.equal(context.normalizeEmployeeCode("１２３４５"), "12345");
assert.equal(context.normalizeEmployeeCode("12a34-56"), "123456");
assert.equal(context.isValidEmployeeCode("12345"), true);
assert.equal(context.isValidEmployeeCode("１２３４５"), true);
assert.equal(context.isValidEmployeeCode("1234"), false);
assert.equal(context.isValidEmployeeCode("123456"), false);

assert.match(indexSource, /id="employeeCode"[^>]+pattern="\[0-9\]\{5\}"/s);
...
assert.match(historySource, /limit\(500\)/);
assert.match(historySource, /exportCsv/);
assert.match(rulesSource, /employeeCode\.matches\('\^\[0-9\]\{5\}\$'\)/);
